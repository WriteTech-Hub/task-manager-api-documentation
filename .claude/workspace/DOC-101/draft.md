...
| POST | `/:taskId/comments` | Add a comment to a task | Yes | Admin / Member |
| GET | `/:taskId/comments` | List all comments on a task | Yes | Admin / Member |
| DELETE | `/:taskId/comments/:commentId` | Delete a comment | Yes | Admin / Member (if author) |

<!-- VERIFY: the source ticket does not state whether Member access to the two read/write comment endpoints above is scoped to tasks the Member created or is assigned to, matching the task-level access rule described earlier on this page. Listed here as unrestricted Admin/Member pending confirmation. -->
...
A comment contains the following fields:

```json
{
  "id": "65f96a1c2b3d44",
  "text": "Looks good, approving now.",
  "authorId": "65f93b8c9e9a2d52c3a421de",
  "createdAt": "2025-01-22T10:15:00.000Z"
}
```

<!-- GAP: every other resource on this page (tasks, checklist items) and in docs/api/users.md uses `_id` as its identifier field. The comment shape above uses `id`, per the source ticket; confirm whether this is intentional or should be `_id` for consistency. -->
...
<!-- GAP: no confirmed maximum length for `text`, and no documented 400 response for missing or empty `text`. Existing task endpoints on this page document 400 for invalid/missing fields; the source ticket does not confirm this applies to comments. -->
...
<!-- VERIFY: response shown as a bare comment object, matching the source ticket's "Returns 201 with the created comment." Other endpoints on this page wrap created/updated resources in a `{ "message": ..., "task": {...} }` envelope — confirm whether comments follow that pattern instead. -->
...
Retrieves all comments on a task, oldest first.
...
Deletes a comment. Only the comment's author or an Admin can delete it.
...
<!-- VERIFY: success response body inferred from the pattern used by "5. Delete Task" on this page ({ "message": ... }). The source ticket states only "Returns 200 on success" without specifying a body. -->

### Response (403 Forbidden)

```json
{
  "message": "Access denied, not authorized to delete this comment"
}
```

<!-- VERIFY: 403 response body inferred from the style of the Common Error Responses table below and the 403 example in docs/api/authentication.md ({"message": "Access denied, admin only"}). The source ticket confirms only the 403 status code, not the message text. -->

---

<!-- GAP: not confirmed whether a distinct "Comment not found" 404 case exists for these endpoints, separate from the "Task not found" case already listed below. Not added to the table without confirmation. -->
